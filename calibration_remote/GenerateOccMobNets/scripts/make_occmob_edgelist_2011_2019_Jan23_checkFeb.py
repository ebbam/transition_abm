...
dict_occ_empincimp = dict(zip(df_emp_incimp["OCC"], \
    df_emp_incimp["ASECWT"]))

dict_occ_empincimp_yearly = dict(zip(df_emp_incimp["OCC"], \
...
df_edgelist["transition_adj_networkers"] = df_edgelist["transition_prob_adj"] * df_edgelist["EMPOCCLY_unadj"]

print("check probs sum to one ", np.isclose(df_edgelist.groupby("OCCLY")["transition_prob_adj"].sum(), 1).sum() == \
    len(df_edgelist.groupby("OCCLY")["transition_prob_adj"].sum()))
...
# Checking self_loops
np.min(df_edgelist["transition_prob"][df_edgelist["OCC"] == df_edgelist["OCCLY"]])

plt.hist(df_edgelist["transition_prob"][df_edgelist["OCC"] == df_edgelist["OCCLY"]],bins=50)
plt.show()
