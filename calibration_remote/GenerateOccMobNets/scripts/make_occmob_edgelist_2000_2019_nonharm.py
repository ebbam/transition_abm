...
# After removing imputations get new df
df_asec_compress = copy.deepcopy(df_asec)
df_asec_compress = df_asec_compress.drop(columns=["PERNUM", "MONTH", "QIND",\
    "IND", "OCC1990", "OCC50LY", "INDLY"])
df_asec_compress.to_csv(path_data + "full_edgelist_clean_labforce_quality.csv")
...
# export
df_emp.to_csv(path_data + "emp_asec_2001_2019_excludeimputations_nonharm.csv", \
    index=False)

# dictionary with employment
dict_OCCLY_emp_yearly = dict(zip(df_emp["OCCLY"], df_emp["ASECWT_average_years"]))
dict_OCCLY_emp = dict(zip(df_emp["OCCLY"], df_emp["ASECWT"]))
...
dict_occ_emp = dict(zip(df_emp_occ["OCC"], df_emp_occ["ASECWT"]))
dict_occ_emp_yearly = dict(zip(df_emp_occ["OCC"], df_emp_occ["ASECWT_average_years"]))
...
df_edgelist.to_csv(path_data + "edgelist_qualitycontrol_2001_2019_nonharm.csv", index=False)

print("only occupation 9830and 9999 dont sum up to one", \
    df_edgelist.groupby("OCCLY")["transition_prob_adj"].sum())
...
df_edgelist[df_edgelist['OCC'] == 9999]
df_edgelist["ASECWT"].loc[df_edgelist['OCCLY'] == 9830].sum()
