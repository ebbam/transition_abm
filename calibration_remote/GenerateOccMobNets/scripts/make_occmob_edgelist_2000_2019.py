...
#TODO correct to use full employment rather than yearly
...
file_asec = "cps_asec_imput_2000.csv"
...
# a = set(df_asec["OCC2010"].unique())
# b = set(df_asec["OCC10LY"].unique())
# b.difference(a)
unemployed_occ_codes = [0, 9840]
df_asec = df_asec[df_asec["EMPSTAT"].isin(employed_codes)]
df_asec = df_asec[~df_asec["OCC10LY"].isin(unemployed_occ_codes)]
...
df_emp_incimp = df_asec.groupby("OCC2010")["ASECWT"].sum().reset_index()
df_emp_incimp["ASECWT"].sum()/(1e6*len(years))

df_emp_incimp["ASECWT_average_years"] = df_emp_incimp["ASECWT"]/len(years)
# export
df_emp_incimp.to_csv(path_data + "emp_asec_2001_2019_includeimputations.csv", \
    index=False)
dict_occ_empincimp = dict(zip(df_emp_incimp["OCC2010"], \
    df_emp_incimp["ASECWT_average_years"]))

# removing imputations and taking quality controls
df_asec = df_asec[(df_asec['UH_SUPREC_A2']==1) | (df_asec['UH_SUPREC_A1']==1)]
# df_asec = df_asec[df_asec["QOCC"]==0]
# df_asec = df_asec[df_asec["QOCC10LY"]==0]
...
# get edgelist
df_edgelist = df_asec.groupby(['OCC10LY', "OCC2010"])["ASECWT"].sum().reset_index()
df_edgelist = df_asec.groupby(['OCC10LY', "OCC2010"])["ASECWT"]\
...
df_edgelist["OCC10LY_label"] = df_edgelist["OCC10LY"].map(dict_code_name)
df_edgelist["OCC_label"] = df_edgelist["OCC2010"].map(dict_code_name)

# get employment
# grouping by employment
df_emp = df_asec.groupby("OCC10LY")["ASECWT"].sum().reset_index()
# export employment without imputations to csv
df_emp["ASECWT_average_years"] = df_emp["ASECWT"]/len(years)
# export
df_emp.to_csv(path_data + "emp_asec_2001_2019_excludeimputations.csv", \
    index=False)

# dictionary with employment
dict_OCC10LY_emp_yearly = dict(zip(df_emp["OCC10LY"], df_emp["ASECWT_average_years"]))
dict_OCC10LY_emp = dict(zip(df_emp["OCC10LY"], df_emp["ASECWT"]))
# now calculate average employment per year
df_emp_occ = df_asec.groupby("OCC2010")["ASECWT"].sum().reset_index()
df_emp_occ["ASECWT_average_years"] = df_emp_occ["ASECWT"]/len(years)
dict_occ_emp = dict(zip(df_emp_occ["OCC2010"], df_emp_occ["ASECWT"]))
dict_occ_emp_yearly = dict(zip(df_emp_occ["OCC2010"], df_emp_occ["ASECWT_average_years"]))

df_edgelist["EMPOCC10LY_unadj"] = df_edgelist["OCC10LY"].map(dict_OCC10LY_emp)
df_edgelist["EMPOCC_unadj"] = df_edgelist["OCC2010"].map(dict_occ_emp)
df_edgelist["EMPOCC_incimp"] = df_edgelist["OCC2010"].map(dict_occ_empincimp)
# calculate transition prob (occ mobility rate)
df_edgelist["transition_prob"] = df_edgelist["ASECWT"]/df_edgelist["EMPOCC10LY_unadj"] 
# adjusting mobility upwards by 55% as recomended
df_edgelist["transition_prob_adj"] = np.where(df_edgelist["OCC2010"] == df_edgelist["OCC10LY"],\
...
df_edgelist.to_csv(path_data + "edgelist_qualitycontrol_2001_2019.csv", index=False)

print("only occupation 9830and 9999 dont sum up to one", \
    df_edgelist.groupby("OCC10LY")["transition_prob_adj"].sum())

df_edgelist

dict_OCC10LY_emp
dict_OCC10LY_emp[9830]

df_edgelist[df_edgelist['OCC2010'] == 9999]
df_edgelist["ASECWT"].loc[df_edgelist['OCC10LY'] == 9830].sum()


# Playing around with the data
df_mobility = df_edgelist[df_edgelist["OCC10LY"] != df_edgelist["OCC2010"]]
...
df_edgelist.loc[df_edgelist["OCC10LY"] == occ_focus]
df_edgelist.loc[df_edgelist["OCC2010"] == occ_focus]

occ_focus = 7440 # 
df_edgelist.loc[df_edgelist["OCC10LY"] == occ_focus]
df_edgelist.loc[df_edgelist["OCC2010"] == occ_focus]
df_edgelist.loc[df_edgelist["OCC10LY"] == occ_focus].sort_values("transition_prob", \
    ascending=False)
...
df_edgelist.loc[df_edgelist["OCC10LY"] == occ_focus].sort_values("transition_prob", \
    ascending=False)
df_edgelist.loc[df_edgelist["OCC2010"] == occ_focus].sort_values("incoming_prob", \
    ascending=False)

# where do they come from
df_edgelist.loc[df_edgelist["OCC2010"] == occ_focus].sort_values("ASECWT", \
    ascending=False)
