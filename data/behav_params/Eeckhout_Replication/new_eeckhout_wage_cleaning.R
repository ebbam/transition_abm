...
library(janitor)
library(here)
# Ensures we only import IPUMS data once
if(!exists("ddi_17_24", envir = .GlobalEnv)){
  ddi_17_24 <- read_ipums_ddi(here("data/behav_params/Eeckhout_Replication/cps_data/cps_00012.xml"))
  data_17_24 <- read_ipums_micro(ddi_17_24) %>%
...
# (b) process_set(): For each “set” (1, 2, or 3), keep only the relevant months and recode to a 1–5 scale.
...
# Extract "first month" wage (renamed previous_wage) and "new wage" (renamed new_wage).
process_first_month <- function(df_set) {
  df_first <- df_set %>%
    filter(month == 1, labforce != 0, earnweek != 9999.99, !is.na(earnweek)) %>%
    rename(previous_wage = earnweek, 
           prev_wtfinl = wtfinl) %>%
    arrange(cpsidp) %>% 
    select(cpsidp, previous_wage, prev_wtfinl)
  return(df_first)
}

process_new_wage <- function(df_set) {
  df_new <- df_set %>%
    filter(month == 5, earnweek != 9999.99, !is.na(earnweek)) %>%
    rename(new_wage = earnweek,
           new_wtfinl = wtfinl) %>%
    arrange(cpsidp) %>% 
    select(cpsidp, new_wage, new_wtfinl)
  return(df_new)
}

# (d) process_ee_flows(): Final selection for EE flows.
# Restrict sample via duplicate tagging and compute employer-switch conditions.
process_ee_flows <- function(df_set, df_new, df_first) {
  df_flow <- df_set %>%
    filter(month %in% c(4, 5)) %>%       # Retain the two months for EE comparisons.
    mutate(month = if_else(month == 4, 1, 2)) %>%
    filter(labforce != 0) %>%
    group_by(cpsidp) %>%
    filter(n() == 2) %>%               # Keep only individuals observed in both months.
    ...
    ungroup() %>%
    filter(diffEEm == 1) %>%
    select(cpsidp, wtfinl, earnweek, earnwt)
  
  df_first %>% 
    left_join(df_flow, ., by = "cpsidp") %>%
    left_join(df_new, ., by = "cpsidp") %>% 
...
# (g) compute_wage_stats(): For a given threshold (multiplier 1, 1.05, or 1.1), compute weighted mean and median.
compute_wage_stats <- function(df, threshold_multiplier) {
  df_sub <- df %>%
    filter(previous_wage * threshold_multiplier < new_wage,
           ...
           # Added these to take care of NA values
           !is.na(previous_wage), !is.na(new_wage)
           )
  ...
  list(avg = avg_wage, median = median_wage, obs = nrow(df_sub))
}

# (h) compute_ee_percentages(): Compute weighted percentage of EE moves.
...
# Processing year 2017 and month index 2
# RUN FROM 2017
...
for(i in 2016:2017) {
  for(j in 1:12) {
    ...
    # Use the master data (data is already the full dataset from 1996-2016)
    df_master <- data
    
    # Apply recoding filter for current i and j.
    df_recoded <- recode_filter(df_master, i, j)
    
    if(is.null(df_recoded)){
      print(paste0("Skipped loop when year = ", i, " and month = ", j, "."))
      next
    }
    
    # Process the three sets (with different month selections and recodings)
    df_set1 <- process_set(df_recoded, 1)
    df_set2 <- process_set(df_recoded, 2)
    df_set3 <- process_set(df_recoded, 3)
    
    # Extract wages for the first month and new wage from each set.
    df_first1 <- process_first_month(df_set1)
    df_new1   <- process_new_wage(df_set1)
    df_ee1    <- process_ee_flows(df_set1, df_new1, df_first1)
    print("set1 done")
    
    df_first2 <- process_first_month(df_set2)
    df_new2   <- process_new_wage(df_set2)
    df_ee2    <- process_ee_flows(df_set2, df_new2, df_first2)
    print("set2 done")
    
    df_first3 <- process_first_month(df_set3)
    df_new3   <- process_new_wage(df_set3)
    df_ee3    <- process_ee_flows(df_set3, df_new3, df_first3)
    print("set3 done")
    ...
    # Report the sample size for this iteration.
    #obs_total <- nrow(df_ee_combined) 
    
    # Compute wage summaries for thresholds 1, 1.05, and 1.1.
    ...
    # Augment the combined data with period info and computed summaries.
    df_ee_final <- #df_ee_combined %>%
      data.frame(date  = paste0(i + 1, sprintf("%02d", j)),
             year  = i + 1,
             month = j,
             observations_total = mean(c(stats_x0$obs, stats_x5$obs, stats_x10$obs)),
    ...
    print(df_ee_final)
    result_list[[counter]] <- df_ee_final
    counter <- counter + 1
    
    # Clean up objects to free memory.
    rm(df_master, df_recoded, df_set1, df_set2, df_set3,
       df_first1, df_new1,
       df_ee1,
       df_first2, df_new2,
       df_ee2,
       df_first3, df_new3,
       df_ee3, df_ee_combined)
    gc()
    
    # if(i == 2016 & j ==10){
    #   bind_rows(result_list) %>% 
    #   saveRDS(., here("data/behav_params/Eeckhout_Replication/new_EE_x_temp_16.rds"))
    #}
  }
}
...
# Save as an RDS file to the specified path.
#saveRDS(final_results, here("data/behav_params/Eeckhout_Replication/new_EE_x.rds"))
saveRDS(final_results, here("data/behav_params/Eeckhout_Replication/new_EE_x_supp.rds"))
readRDS(here("data/behav_params/Eeckhout_Replication/new_EE_x_temp_16.rds")) %>% 
  bind_rows(readRDS(here("data/behav_params/Eeckhout_Replication/new_EE_x_supp.rds"))) %>% 
  bind_rows(readRDS(here("data/behav_params/Eeckhout_Replication/new_EE_x.rds"))) %>% tibble() %>% distinct %>% saveRDS(., here("data/behav_params/Eeckhout_Replication/EE_x_final.rds"))

# Needed to run loop for values before and after 2016 due to IT issues - bind the two dataframes below:

new <- readRDS(here("data/behav_params/Eeckhout_Replication/new_EE_x_temp_16.rds")) %>% 
  bind_rows(readRDS(here("data/behav_params/Eeckhout_Replication/new_EE_x_supp.rds"))) %>% 
  bind_rows(readRDS(here("data/behav_params/Eeckhout_Replication/new_EE_x.rds"))) %>% tibble() %>% distinct
  tibble() %>% 
  select(-year, -month) %>%
...
old <- read_dta(here("data/behav_params/Eeckhout_Replication/116422-V1/data/EE_x/EE_x.dta")) %>% 
  select(-c(year, month, cpsidp, previous_wage, new_wage, earnwt, wtfinl_1)) %>%
  mutate(date = ifelse(nchar(date) == 5, paste0(substr(date, 1, 4), "0", substr(date, 5,5)), date),
         date = as.numeric(date)) %>%
  pivot_longer(!date) %>% 
  mutate(cat = "old") %>% 
  # multiple observations in 199701 with identical values
  distinct

#readRDS(here("data/behav_params/Eeckhout_Replication/new_EE_x.rds")) %>% tibble() 

new %>% 
  bind_rows(old) %>% filter(is.na(value)) %>% pull(date) %>% unique
  ggplot() + 
  geom_line(aes(x = date, y = value, color = cat)) + facet_wrap(~name, scales = "free")

# message("Process complete. Final aggregated data saved to 'data/behav_params/Eeckhout_Replication/new_EE_x.dta'")
