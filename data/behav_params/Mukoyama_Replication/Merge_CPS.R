...
if(new){
  ...
  for(y in 2023:2024){
    ...
    filenames <- list.files(here(base_new), pattern = paste0("*intermediate_", as.character(y))) %>% 
    ...
    saveRDS(cps_data_new, here(paste0("data/behav_params/Mukoyama_Replication/mukoyama_all/final_data/R_final/cps_data_no_time_temp_", as.character(y), ".rds")))
...
for(y in 2016:2024){
  ...
  # Appending all files with "*intermediate*" pattern
  filenames <- list.files(here(base_new), pattern = paste0("*no_time_temp_", as.character(y))) %>% 
    paste0(base_new, .)
  ...
  saveRDS(cps_data, here(paste0(base_new, "cps_data_no_time_", as.character(y), ".rds")))
  print(paste0("Saved: ", as.character(y)))
  }
}
...
# 
# # Get the last observation values for weights
# last_obs <- nrow(data) # Assuming `data` is the dataframe containing all rows
# 
# # Extract the pweights and sweights for the last observation
# for (var in variables) {
#   assign(paste0("pweight_", var), data[[paste0("pweight_", var)]][last_obs])
#   assign(paste0("sweight_", var), data[[paste0("sweight_", var)]][last_obs])
# }
# 
# # Get sweight_invmills and sigma_hat from the last observation
# sweight_invmills <- data$sweight_invmills[last_obs]
# sigma_hat <- data$sigma_hat[last_obs]
# 
...
#     time_create = psearch * search_cond
#   )
# 
# # Replace employed searchers' search time with NA
# data <- data %>%
#   mutate(time_create = ifelse(mlr == 1 | mlr == 2, NA, time_create))
# 
