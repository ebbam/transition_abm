...
if(first){
  ...
  for(i in 2015:2024){
    print(i)
    data <- readRDS(here(paste0(base, "final_data/R_final/cps_data_full_", as.character(i), ".rds"))) 
    ...
    data1 <- data %>% 
     ...
     add_years <- rbind(add_years, data1)
     
     data2 <- data %>% 
       filter(unemp == 1) %>%
       group_by(year, month) %>%
       summarise(
         num_search = weighted.mean(num_search, newwgt, na.rm = TRUE),
         time_create = weighted.mean(time_create, newwgt, na.rm = TRUE)
       ) %>%
       ungroup()
     
     add_years_unemp <- rbind(add_years_unemp, data2)
  ...
  collapsed_data_3a <- data_fig3_mutated %>%
    group_by(year, month) %>%
...
# ##############################################################################
  ...
  # Apply the seasonal adjustment to each variable
  for (var in c("num_search", "time_create")) {
    collapsed_unemp_adj <- seasonal_adjust(collapsed_unemp_adj, var)
  }

 #write.csv(collapsed_unemp_adj, here(paste0(base, "final_data/R_final/Figure3b_data_extended.csv")))
}
