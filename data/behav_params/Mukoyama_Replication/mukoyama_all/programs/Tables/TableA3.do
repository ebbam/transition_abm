...
/********************************************************
** Description: This program calculates Table A3 (Appendix Table 3)  
**********************************************************/
...
*********************************Table A3: The Long Version of the Full Sample Regression 

file open mytable using "$tables/TableA3.tex", write text replace
...
file write mytable "\begin{tabular}{l*{6}{c}} \label{tab:full_sample}" _newline
file write mytable "\\" _newline
file write mytable "\hline \hline" _newline
file write mytable "&& \multicolumn{2}{{c}}{Composite Help-Wanted Index (1994-2014)} && \multicolumn{2}{{c}}{JOLTS (2001-2014)} \\" _newline
file write mytable "\\" _newline
file write mytable " && Basic & Observables  && Basic & Observables  \\" _newline
file write mytable "\cline{3-4} \cline{6-7}" 
file write mytable "\\" _newline
file close mytable
...
foreach var of local variable_list {
	...
	estpost tabstat beta_nop_nox_BAR beta_nop_BAR  ///
			beta_nop_nox_JOLTS beta_nop_JOLTS if xvar=="`var'" , statistics(mean) missing

	estout . using "$tables/TableA3.tex" ,  append ///	
	style(tex) ///
	cells("beta_nop_nox_BAR(fmt(3)) beta_nop_BAR(fmt(3)) beta_nop_nox_JOLTS(fmt(3)) beta_nop_JOLTS(fmt(3))") /// 
	mlabels(none) collabels(none) ///
	varlabels(mean "`label'") /// 
	extracols(1 3) varwidth(36) modelwidth(0 14 0 14)
	
	estpost tabstat se_nop_nox_BAR se_nop_BAR  ///
			se_nop_nox_JOLTS se_nop_JOLTS if xvar=="`var'" , statistics(mean) missing

	estout . using "$tables/TableA3.tex" ,  append ///	
	style(tex) ///
	cells("se_BAR_nop_nox_hps(fmt(3) par) se_nop_BAR(fmt(3) par) se_nop_nox_JOLTS(fmt(3) par) se_nop_JOLTS(fmt(3) par)") /// 
	mlabels(none) collabels(none) ///
	varlabels(mean " ") /// 
	extracols(1 3) varwidth(36) modelwidth(0 14 0 14)	
	...
	file open mytable using "$tables/TableA3.tex", write text append
	file write mytable "\\" _newline
	file close mytable
...
estpost tabstat N_nop_nox_BAR N_nop_BAR  ///
			N_nop_nox_JOLTS N_nop_JOLTS if xvar=="lcycle" , statistics(mean) missing

estout . using "$tables/TableA3.tex" ,  append ///	
style(tex) ///
cells("N_nop_nox_BAR N_nop_BAR N_nop_nox_JOLTS N_nop_JOLTS") /// 
mlabels(none) collabels(none) ///
