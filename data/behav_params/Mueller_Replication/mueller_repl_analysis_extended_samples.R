...
prod_fig1 <- function(data, title){
  ...
  p <- data_fig1 %>% 
    ...
    labs(
      ...
      title = paste0(title, ": Figure 1. Histograms of Elicited Job-Finding Probabilities - Panel A. SCE (3-mo horizon)")
    ) +
    scale_y_continuous(
      limits = c(0, 3),
...
prod_fig1 <- function(df_list) {
  ...
  p <- ggplot(combined_data, aes(x = find_job_3mon, weight = fweight, fill = Source, color = Source)) + 
    ...
    labs(
      y = "Density",
      x = "Elicited 3-Month Job-Finding Probability",
      title = "Density Comparison of Elicited Job-Finding Probabilities"
    ) + 
    scale_y_continuous(
      limits = c(0, 3), 
      breaks = seq(0, 2, by = 0.5)
...
ggplot(combined_data, aes(x = x_adjusted-0.02, 
  ...
  theme_minimal() +
  theme(legend.title = element_blank(), major.gridlines = element_blank())
...
modelB_2 <- function(data_tab2){  # The next regressions use same sample as in the above regression which rules out any NA values for tplus3_UE_trans_3mon and tplus3_percep_3mon
  ...
  lfe::felm(as.formula(paste0("tplus3_UE_trans_3mon ~ ", paste0(controls, collapse = " + "), " + (1 | userid)")), data = data_tab2_short, weights = data_tab2_short$weight)%>% return(.)}
...
# Plot the data
ggplot(combined_data_fig3, aes(x = undur)) +
  # Elicited probabilities with error bars
...
prod_fig4_multi <- function(df_list) {
  ...
  print(term_levels)
  print(combined_data) %>% print(n = 50)
  
  
  # Plot the data
  plot <- combined_data %>%
    filter(term %in% grep("^nedur_1mo_", names(df_list[[1]]), value = TRUE)[1:7]) %>%
    ...
    labs(color = "Sample",
         ...
         title = "Changes in Job-Finding Probability Across and Within Spells") +
    theme_minimal() +
