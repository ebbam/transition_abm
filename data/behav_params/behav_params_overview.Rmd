...
# A Calibrating Behavioural Mechanisms {.tabset}
...
## Behavioural Mechanism Calibration
...
decision-making on the job search process.

In nearly all cases, the results are "binned" into intervals (ie. number
of people sending 81 or more applications or unemployment duration of
between 5 and 14 weeks) which means that any line plots (or linear
interpretation of the bar graph) should be done carefully. Preliminary
...
```{r cps_supp1, echo = FALSE, include = TRUE, cache = TRUE, echo = FALSE, messages = FALSE, warning=FALSE}
...
```{r cps_supp2, echo=FALSE, fig.height = 7, fig.width = 10, cache = FALSE}
...
*Preliminary hypothesis:* Best fit will be with a complementary log-log
as we care more about distinguishing between lower-level bins and there
are few observations in the highest-level bins.

```{r cps_supp3, echo=FALSE, fig.height = 6, fig.width = 10, cache = FALSE}
...
*Result:* For each additional quarter of unemployment, an individual’s
...
```{r cps_supp4, echo=FALSE, fig.height = 6, fig.width = 8, cache = FALSE}
...
```{r eeckhout, echo = FALSE, fig.height = 8, fig.width = 10, cache = FALSE, results = 'asis'}
...
4.  (Validation) Mukoyama et al. data on the intensive margin of
    unemployed search effort (in minutes searched) over the business
    cycle. We have chosen to include this as a validation exercise of
    our application effort imposition.
...
agents. I will provide more explanation on this...

(Replicated with additional data for unemployed jobseekers) [**Mukoyama
et al. 2018: Job Search Over the Business
...
```{r mukoyama, echo = FALSE, fig.height = 8, fig.width = 10, cache = FALSE}
