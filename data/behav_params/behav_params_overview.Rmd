...
# Determinants of job-finding:
- Dynamic selection (heterogeneity of job seekers - fitness & skills match for the current labour market)
- Duration dependence (job-finding probability decreases over time spent unemployed)
...
s_theta_df <- read.delim(here("data/behav_params/Mukoyama_Replication/programs/Figures/s_theta.m"), sep = ",") %>% 
  tibble() %>%
  rename(s_theta = 1) %>% 
...
cps_minutes <- read.csv(here("data/behav_params/Mukoyama_Replication/int_data/CPS/FigureA4_data.csv")) %>% 
  tibble() %>% 
